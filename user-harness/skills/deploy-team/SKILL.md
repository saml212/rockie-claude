...
3. Invoke from the repo you're working in:
   ```bash
   node "$HOME/.claude/teams/orchestrator/index.js" <path/to/config.json>
   ```
...
## Config (authoritative at `$HOME/.claude/teams/schema.json`)
...
- Node.js ≥ 20
- `express` pre-installed at `$HOME/.claude/teams/orchestrator/node_modules/`
