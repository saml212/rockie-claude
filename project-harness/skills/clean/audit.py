...
On zero-blocker pass, hands off to .claude/scripts/clean-finalize.sh,
which writes the sentinel .claude/.state/clean-ok-<hash> AND emits the
...
def main():
    ...
    if not blockers:
        ...
        if h and h != "no-changes":
            ...
            # agent's context.
            r = subprocess.run(
                ["bash", ".claude/scripts/clean-finalize.sh", h],
