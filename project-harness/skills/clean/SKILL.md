...
3. If zero blockers: the audit invokes
   `.claude/scripts/clean-finalize.sh <hash>` as its final action.
   That script writes `.claude/.state/clean-ok-<hash>` (where
   `<hash>` is computed by `.claude/scripts/compute_clean_hash.sh`)
...
```bash
bash .claude/scripts/clean-finalize.sh <hash>
```

`audit.py` already invokes this for the agent. The script:

1. Writes the audit sentinel at `.claude/.state/clean-ok-<hash>` in
