...
# upstream-contribute nudge to stderr so Claude Code surfaces it to the
# agent's context every time the audit passes. The nudge is no longer
# prose-only in SKILL.md — putting it here makes it deterministic.
#
# Usage:
#   clean-finalize.sh <staged-hash>
#
# Called from skills/clean/audit.py after a zero-blocker pass. Sentinel
# format must stay compatible with hooks/pre-commit-gate.sh (touch-style
# marker file at .claude/.state/clean-ok-<hash>).
set -u
...
# Nudge — printed to stderr so Claude Code surfaces it as a hook message
# the agent can't silently ignore. One short paragraph; the agent decides
# whether to act on it. Never auto-invokes /upstream-contribute.
cat >&2 <<'NUDGE'
[clean] audit clean. anything in this session worth upstreaming to
saml212/rockie-claude? (skills, hooks, memory schema improvements,
